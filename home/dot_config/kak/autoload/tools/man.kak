...
define-command -docstring 'Go to next man page link' man-link-next %{ man-search %opt[man_link2] n }

define-command -docstring 'Go to previous man page link' man-link-prev %{ man-search %opt[man_link2] <a-n> }

define-command -docstring 'Try to jump to a man page' man-jump %{
...
define-command kan -docstring ' kan <query>: Either show a man-page or the --help text for the given query' -shell-completion -params 0.. %{
  evaluate-commands %sh{
...
define-command kelp -docstring 'kelp <command>: --help output utility' -shell-completion -params 0.. %{
  evaluate-commands %sh{
    if [ $# -eq 0 ] && "$kak_selection" --help > /dev/null; then
      echo "fifo -name "help" -- \"$kak_selection\" --help"
    elif [ $# -eq 0 ] && "$kak_selection" -h > /dev/null; then
      echo "fifo -name "help" -- \"$kak_selection\" -h"
    elif "$*" --help > /dev/null; then
      echo "fifo -name "help" -- \"$*\" --help"
    elif "$*" -h > /dev/null; then
      echo "fifo -name "help" -- \"$*\" -h"
    fi
  }
}
