provide-module fifo %{
    define-command -params .. -docstring %{
        ...
        pidfile=$(dirname ${output})/pid
        mkfifo ${output}
        if [ -n "$script" ]; then
            ( eval "$script" > ${output} 2>&1 & echo $! > ${pidfile} ) > /dev/null 2>&1 < /dev/null &
        else
            ( "$@" > ${output} 2>&1 & echo $! > ${pidfile} ) > /dev/null 2>&1 < /dev/null &
        fi
        ...
        printf %s\\n "
                ...
                hook -always -once buffer BufCloseFifo .* %{ nop %sh{
                    # Kill the process and all its children if PID file exists
                    if [ -f ${pidfile} ]; then
                        pid=\$(cat ${pidfile})
