provide-module wezterm %{
  ...
  define-command split -params 0..1 -docstring "split [file]: Split current pane horizontally" %{
    evaluate-commands %sh{
      if [ $# -eq 0 ]; then
        echo "nop %sh{ wezterm cli split-pane --horizontal -- kak -c '$kak_session' '$kak_bufname' }"
      else
        echo "nop %sh{ wezterm cli split-pane --horizontal -- kak -c '$kak_session' '$1' }"
      fi
    }
  }
  ...
  define-command vsplit -params 0..1 -docstring "vsplit [file]: Split current pane vertically" %{
    evaluate-commands %sh{
      if [ $# -eq 0 ]; then
        echo "nop %sh{ wezterm cli split-pane --bottom -- kak -c '$kak_session' '$kak_bufname' }"
      else
        echo "nop %sh{ wezterm cli split-pane --bottom -- kak -c '$kak_session' '$1' }"
      fi
    }
  }
