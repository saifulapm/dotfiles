...
// projection with its substring filter. Only whitespace changed, to house
// 4-space qmlformat.
...
// Newest first. An entry already in the history is not duplicated — it is
// simply dropped from the tail as the list is rebuilt around the new head, so
// re-copying something moves it to the top.
function addEntry(history, entry, limit) {
    ...
    max = Math.max(0, max);
    if (!normalized)
        return Array.isArray(history) ? history.slice(0, max) : [];
    if (max === 0)
        return [];
