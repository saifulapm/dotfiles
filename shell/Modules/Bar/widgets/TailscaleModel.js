...
// list, the Taildrop capability tests, and the login plan.
//
// Every glyph here is already a Material Design codepoint (U+F0001 and up), so
// they survive our Symbols Nerd Font fallback unchanged — the FontAwesome,
// Devicons and Codicon ranges do not. Only the panel's account row needed a
// substitution; see TailscalePanel.qml.
...
function accountLabel(account) {
    ...
    if (account.nickname)
        return String(account.nickname);
    if (account.tailnet)
        return String(account.tailnet);
    if (account.account)
...
function hasFileSharing(self) {
    ...
    if (capMap && capMap[capability] !== undefined)
        return true;
    var capabilities = (self && self.Capabilities) || [];
    for (var i = 0; i < capabilities.length; i++) {
        if (String(capabilities[i]) === capability)
            return true;
    }
    return false;
}
...
// `tailscale exit-node list` prints a fixed-column table, so the header's own
// column offsets are what split the rows.
function parseExitNodeList(raw) {
    ...
    var result = [];
    for (var hostName in byHost)
        result.push(byHost[hostName]);
...
// One row per city rather than per Mullvad host: the picker offers regions,
// and tailscale picks a host inside the one that is chosen.
function mullvadRegionOptions(nodes) {
    ...
    for (var i = 0; i < values.length; i++) {
        ...
        for (var propertyName in node)
            option[propertyName] = node[propertyName];
    ...
    var result = [];
    for (var name in byRegion)
        result.push(byRegion[name]);
    result.sort(function (a, b) {
        var countryCompare = String(a.Country).localeCompare(String(b.Country));
        if (countryCompare !== 0)
            return countryCompare;
    ...
    try {
        ...
        for (var id in rawPeers) {
            ...
            if (normalized.Online) {
                ...
                if (normalized.ExitNodeOption)
                    exitNodes.push(normalized);
            }
        }
...
// Ours, not theirs: tailscale refuses `up`, `down`, `set` and profile reads
// for a user who is not the configured operator, and it says so in prose on
// stderr rather than with a distinguishable exit code. Upstream only reads the
// one wording their account list produces ("profiles access denied"); every
// other privileged refusal reaches their panel as raw stderr and no hint about
// what to do. This widens the test so the authorize row appears for any of
// them.
