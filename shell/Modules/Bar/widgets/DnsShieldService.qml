...
QtObject {
    ...
    readonly property Process probeProcess: Process {
        running: false
        command: ["setpriv", "--pdeathsig", "TERM", "--", "bash", "-c", "systemctl cat ublockdns.service >/dev/null 2>&1 || exit 3; " + "echo client=$(systemctl is-active ublockdns 2>/dev/null); " + "echo dnsmasq=$(systemctl is-active dnsmasq 2>/dev/null); " + "b=$(ss -lnu 2>/dev/null); " + "case \"$b\" in *'127.0.0.1:53 '*) echo bind_client=yes;; *) echo bind_client=no;; esac; " + "case \"$b\" in *'127.0.0.2:53 '*) echo bind_fwd=yes;; *) echo bind_fwd=no;; esac; " + "ips=$(grep -Eo '^[0-9][0-9.]*' \"$HOME/.config/dns-helper/lan-ip\" 2>/dev/null | tr '\\n' ' '); " + "echo lan_ips=$ips; " + "bound=; for ip in $ips; do case \"$b\" in *\"$ip:53 \"*) bound=\"$bound$ip \";; esac; done; " + "echo lan_bound=$bound; " + "dev=$(ip route get 1.1.1.1 2>/dev/null | sed -n 's/.*dev \\([^ ]*\\).*/\\1/p' | head -1); " + "echo laptop_dns=$(resolvectl status $dev 2>/dev/null | sed -n 's/.*Current DNS Server: //p' | head -1); " + "echo block_test=$(dig @127.0.0.1 youtube.com +short +time=1 +tries=1 2>/dev/null | head -1)"]
