...
// flags enum, so the QML resolves it with PwNodeType.toString() first — the
// AudioModel.js precedent), and whitespace follows house 4-space style. The
// module.exports tail is kept so the ledger and matching rules can be
...
function canHandleAction(player, action) {
    ...
    if (action === "play")
        return !!(player.canPlay || player.canTogglePlaying)
    if (action === "pause")
        return !!(player.canPause || player.canTogglePlaying)
    if (action === "playPause")
...
function osdMessage(player, fallback) {
    ...
    if (label && player.trackArtist)
        return label + " - " + player.trackArtist
